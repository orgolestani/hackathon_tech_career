...
class LogIn {
    ...
    async registerUser(
        credentials: { company: string; email: string },
        role: string
    ) {
        const url = `${API}/registration/register/${role}`
        const body = { user: credentials }
    ...
    async signUpUser(
        credentials: { user: object; company: object },
        role: string,
        token?: string
    ) {
    ...
    setTokenSession = (userToken: string) =>
        sessionStorage.setItem(this.token, userToken)
    setTokenLocal = (userToken: string) =>
        localStorage.setItem(this.token, userToken)
